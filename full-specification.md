...
ボール保持者は、まず前方にスペースがあるかを判定する。前方7ユニット以内の40度扇状範囲に敵がいない場合、かつゴールまで12ユニット以上離れている場合、**65%の確率でドリブルを優先選択**する。これにより、スペースがある状況での無理なパスが減少し、よりサッカーらしいドリブル突破が実現される。
...
**優先度2: パス (PASS)** — `bestPass` 関数で最適なパス先を探索。30%の確率でロングパスも検討する。パス先評価スコアは以下のように改善された：

```
goalProgress = -team * receiver.pos.x
gpScore = goalProgress * 0.6  // 前進パス
if goalProgress < 0:  // バックパス
  gpScore = goalProgress * 0.05  // ポゼッション維持のため減点を大幅緩和

score = openness * 2.0 + gpScore - distance * 0.1
if laneBlocked: score -= 6.0  // 無理な縦パスを減らすため強化
