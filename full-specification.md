...
ボール保持者は、まず前方にスペースがあるかを判定する。前方7ユニット以内の40度扇状範囲に敵がいない場合、かつゴールまで12ユニット以上離れている場合、**85%の確率でドリブルを優先選択**する（v4.0で0.65→0.85に引き上げ）。これにより、スペースがある状況での無理なパスが減少し、よりサッカーらしいドリブル突破が実現される。
...
#### 7.4.3 味方がボール保持時（攻撃ラン）— Box-to-Box動き追加

攻撃参加レベル `atkFactor`（0.0〜1.0）に応じてポジショニングが変化する。

| 役割 | 攻撃時の行動 |
|------|------------|
| **MID（Box-to-Box飛び出し）** | **確率 1.5% + af * 4%** でボックス内へ飛び出し、FWDを追い越す動き。攻撃レベルが高いほど頻繁に発生。 |
| MID（通常） | 保持者の斜め後ろ3ユニット、横5ユニットのサポートポジション（リンクマン）。 |
| DEF | 保持者の後方 2 + af * 4 ユニットに位置。カウンター対策でリミット（team < 0 なら x < 2.0）を設定し、上がりすぎを防止。 |
| FWD | 相手ゴール方向 0.6 + af * 0.3 の深さに前線ラン。常に裏を狙う。 |
