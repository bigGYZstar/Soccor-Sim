...
#### 7.2.1 プログレッシブドリブル（運ぶドリブル）の最優先化（v5.0で追加）

ボール保持者は、**パス判定の前に**、ゴール方向が空いているかを判定する。ゴール方向への進行ルート上（30度扇状範囲、探索距離は最大10ユニット）に敵がいない場合、かつシュート距離外（`distToGoal > shotRange`）の場合、**確率ではなく判断として問答無用でドリブル**を実行する。これにより、広大なスペースがあるのにパスコースを探して停滞する問題が解消され、サッカーらしい「運ぶドリブル」が実現される。

```typescript
// ターゲットを「現在の位置からゴール方向へ8ユニット進んだ地点」に設定
...
**優先度2: パス (PASS)** — `bestPass` 関数で最適なパス先を探索。30%の確率でロングパスも検討する。パス先評価スコアは以下のように改善された（v4.0で前進パスボーナスを大幅強化）：

```
goalProgress = -team * receiver.pos.x
gpScore = goalProgress * 3.5  // 前進パス（v4.0で0.6→3.5に強化）
if goalProgress < 0:  // バックパス
  gpScore = goalProgress * 0.05  // ポゼッション維持のため減点を大幅緩和

// アタッキングサード（相手陣地深く）へのパスにさらにボーナス
if isAttackingPass (receiver.x > pitchHalfW * 0.25):
  gpScore += 2.0

score = openness * 1.5 + gpScore - distance * 0.08  // opennessの重みを下げてリスク許容
if laneBlocked:
  // 前方パス（gp > 2.0）なら多少ブロックされていても通る可能性に賭ける
  score -= (goalProgress > 2.0) ? 3.0 : 6.0
if offside: score -= 20  // オフサイドは完全除外
```
...
### 7.4 ボール非保持時の意思決定 (`decideNoBall`) — 完全書き換え
...
#### 7.4.3 味方がボール保持時（攻撃ラン）— ハイライン押し上げ（v5.0で完全書き換え）

攻撃参加レベル `atkFactor`（0.0〜1.0）に応じて、チーム全体のラインが前方に押し上げられる。`pushUpDist = af * 4.5` により、攻撃レベルMAX時には全体ラインが最大4.5ユニット前進する。

| 役割 | 攻撃時の行動 |
|------|------------|
| **DEF（ハイライン）** | ボール位置と自Homeポジションの中間に位置し、攻撃レベルに応じて前方に押し上げ。攻撃レベルMAXなら、CBでもセンターサークル付近まで押し上げる。ただし、カウンター対策で相手陣地深く（`abs(lineX) > pitchHalfW * 0.8`）には入らない。 |
| **MID（Box-to-Box）** | ボールから15ユニット以上離れており、かつ攻撃レベル > 0.5 の場合、ペナルティエリア手前（`attackX = oppGoalX * (0.3 + af * 0.5)`）まで侵入。それ以外はリンクマンとして保持者の斜め後方にサポート。 |
| **FWD** | 常にペナルティエリアエッジ（`penAreaEdge = oppGoalX - (-team * penAreaW)`）付近で裏抜けを狙う。 |

#### 7.4.4 相手がボール保持時（守備）— ゾーンディフェンス

**団子サッカー防止**のため、各選手は自分の担当ゾーン（Homeポジション周辺18ユニット）内でのみ積極的にプレスを行う。

| 役割 | 守備時の行動 |
|------|------------|
| FWD | ゾーン外かつボールが25ユニット以上離れている場合、深追いせずゆっくり戻る（`vlerp(pos, home, 0.05)`）。カウンターに備えてハーフウェーライン付近で待機。 |
| MID/DEF | ボールが8ユニット以内かつゾーン内 → ボールキャリアに直接プレス。それ以外 → ボールと自ゴールの間に入りつつ、Homeポジションとのバランスを保つ（`vlerp(blockPos, home, 0.5)`）。 |
...
12. ボール減速
13. トレイル減衰
14. プレイヤーループ（22人）: インターセプト → 意思決定 → **移動（慣性付き）** → ジャンプ減衰

### 16.2 移動ロジック：慣性（方向転換ペナルティ）の導入（v4.0）

物理エンジンの完全な書き換えを避けつつ、「急な方向転換時に速度が落ちる」挙動を追加し、守備側のオーバーランや攻撃側の切り返しを有効にした。

```typescript
// 移動処理部分（update関数内）
if (p.act !== "idle") {
  const baseSpd = (p.act === "dribble") ? P.dribbleSpeed : P.moveSpeed;
  const dir = p.tgt ? vnorm(vsub(p.tgt, p.pos)) : p.dir;
  
  // 方向転換角度を計算
  const turnAngle = vang(p.dir, dir);  // 0〜180度
  
  // 急旋回時の速度低下（最大40%まで）
  let turnPenalty = 1.0;
  if (turnAngle > 30) {
    turnPenalty = Math.max(0.6, 1.0 - (turnAngle - 30) / 150 * 0.4);
  }
  
  // 守備時はさらに5%減速（攻撃側が剥がしやすく）
  if (p.team !== st.ball.team) turnPenalty *= 0.95;
  
  const finalSpd = baseSpd * turnPenalty;
  
  // 向きも徐々に変化（slerp的挙動）
  p.dir = vnorm(vadd(vmul(p.dir, 0.75), vmul(dir, 0.25)));
  
  // 移動
  p.pos = vadd(p.pos, vmul(p.dir, finalSpd * dt));
}
```

**効果:**
- 守備側がボールキャリアを追い越してオーバーランすると、急旋回で速度が落ちて剥がしやすくなる。
- 攻撃側はドリブル中に切り返してスペースを作れる。
- 守備側の瞬時反応張り付きが抑制され、よりリアルな追跡挙動になる。
