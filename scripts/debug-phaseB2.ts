...
while (!st.over && frames < MAX_FRAMES) {
  ...
  if (st.ball.owner !== null) {
    ...
    if (ax >= 35.0) phaseBFrames++;
  }
}

// Check what pitchHalfW actually is
const { P } = await import('../client/src/game/constants');
process.stderr.write(`pitchHalfW from constants: ${P.pitchHalfW}\n`);
process.stderr.write(`Phase B threshold (2/3 of pitchHalfW): ${(2/3 * P.pitchHalfW).toFixed(2)}\n`);
...
process.stderr.write(`Phase B frames (ax >= 35): ${phaseBFrames}\n`);
